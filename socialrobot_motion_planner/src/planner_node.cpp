...
#include <socialrobot_motion_planner/ArmPlanning.h>

namespace socialrobot_motion_planner
{
...
private:
  ...
  bool armPlanningCallback(ArmPlanningRequest &request, ArmPlanningResponse& response)
  {
    ...
    if(request.arm_type == ArmPlanningRequest::LEFT_ARM)
    {
      ...
      for (int i=0; i<n; i++)
      {
        response.trajectory.joint_names.push_back(std::string("7dof_RISE_joint_"+i+1));
      }
    }
    else if (request.arm_type == ArmPlanningRequest::RIGHT_ARM)
    {
...
using namespace socialrobot_motion_planner;
